...
export default function Checkout() {
  ...
  function checkoutAction(event) {
    ...
    if (errorList.length === 0) {
      setSuccess(true);
      // Optionally: cartCtx.clearCart();
      // Optionally: userProgressCtx.showSuccess();
      console.log("Order submitted successfully!");
    }
  }
  ...
  if (success) {
    return (
      <Modal open={userProgressCtx.showSuccess === 'success'}>
        <h2>Success!</h2>
        <p>Your order was submitted successfully.</p>
        <p>We will notify you with more details through text message in minutes.</p>
        <button type="button" onClick={handleCloseSuccess}>Okay</button>
    ...
    <Modal open={userProgressCtx.progress === "checkout"}>
      <form onSubmit={checkoutAction} className="checkout-form">
        ...
        <div className="checkout-button">
          ...
          <button type="submit" on={handleSubmitOrder}>Submit Order</button>
        </div>
      </form>
    </Modal>
  );
